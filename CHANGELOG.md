...
- Added `scripts/verification_state.py` and its schema. It fingerprints the complete Git change set (committed `base...HEAD`, HEAD/base, statuses, staged/unstaged diffs, and changed-file content), binds the frozen Task Packet identity/bytes/scope, strictly derives the state-file exclusion in the CLI, and returns `UNVERIFIED` when the final snapshot or packet moves or exceeds scope.
